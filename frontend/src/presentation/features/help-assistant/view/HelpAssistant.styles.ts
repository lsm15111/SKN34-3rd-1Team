...
// 치수는 2026-09-12에 intercom.com에서 실제로 재 본 값을 기준으로 맞췄습니다.
// 메신저 400x704, 런처 48, 둘 사이 간격 16, 화면 가장자리 20~35. 우리는 폭 400·높이 720을 쓰되
// 낮은 화면에서는 창을 넘지 않게 줄이고, 런처(52)와의 간격은 같은 16으로 둡니다.
export const helpAssistantStyles = {
  ...
  panel:
    'fixed right-6 bottom-[5.75rem] z-50 flex h-[min(45rem,calc(100dvh-9rem))] w-[25rem] flex-col overflow-hidden rounded-2xl border border-sample-border bg-white text-app-ink shadow-[0_2px_4px_rgb(23_27_25_/_6%),0_24px_56px_-20px_rgb(23_27_25_/_30%)] motion-safe:animate-help-panel-enter max-chat:inset-0 max-chat:h-auto max-chat:w-auto max-chat:rounded-none max-chat:border-0',
  panelLifted: 'bottom-[10rem] max-chat:bottom-0',

  header: 'flex flex-none items-center justify-between gap-2 border-b border-sample-border px-5 py-3.5',
  headerTitle: 'm-0 flex items-center gap-2 text-[0.95rem] font-bold',
  headerDot: 'size-[0.4375rem] flex-none rounded-full bg-brand-primary',
  headerClose:
    'grid size-8 flex-none cursor-pointer place-items-center rounded-full text-[0.95rem] text-sample-muted hover:bg-app-canvas hover:text-app-ink focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-brand-primary',

  body: 'flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto overscroll-contain bg-white px-5 py-5',
  ...
  userBubble:
    'max-w-[86%] rounded-[1.1rem_0.3rem_1.1rem_1.1rem] bg-brand-accent px-4 py-2.5 text-[0.875rem] leading-[1.65] font-medium text-[#0a5c36] break-keep',
