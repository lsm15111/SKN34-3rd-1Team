...
describe('지원사업 직접 필터 검색', () => {
  ...
  it('첫 진입은 페이지 크기만큼 스켈레톤 행을 보여 주고 필터는 바로 쓸 수 있다', async () => {
    const fetchMock = vi.fn().mockImplementation(() => new Promise<Response>(() => {}))
    vi.stubGlobal('fetch', fetchMock)
    start()
    const region = screen.getByRole('region', { name: '필터 검색 결과' })
    await waitFor(() => expect(fetchMock).toHaveBeenCalledOnce())
    const skeleton = region.querySelector('[aria-busy="true"] > [aria-hidden="true"] > div')
    expect(skeleton?.children).toHaveLength(12)
    expect(within(region).getByRole('status').textContent).toContain('공고를 불러오고 있어요')
  ...
  it('조건을 바꾸면 이전 목록을 유지한 채 갱신 중임을 알리고 새 결과로 바꾼다', async () => {
    let resolveSecond!: (response: Response) => void
    const fetchMock = vi.fn().mockResolvedValueOnce(Response.json(catalog))
      .mockImplementationOnce(() => new Promise<Response>((done) => { resolveSecond = done }))
    vi.stubGlobal('fetch', fetchMock)
    start()
    await screen.findByRole('link', { name: program.title })
    fireEvent.click(screen.getByRole('radio', { name: '서울' }))
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(2))
    const region = screen.getByRole('region', { name: '필터 검색 결과' })
    // 이전 행·건수·적용 칩은 그대로이고 결과 영역만 갱신 중으로 표시한다. 필터와 검색 버튼은 잠그지 않는다.
    expect(within(region).getByRole('link', { name: program.title })).toBeTruthy()
    expect(within(region).getByRole('heading', { level: 2 }).textContent).toContain('1건')
    expect(region.querySelector('[aria-busy="true"]')).toBeTruthy()
    expect(within(screen.getByRole('group', { name: '적용된 필터' })).getByText('지역: 서울')).toBeTruthy()
    expect((screen.getByRole('button', { name: '검색' }) as HTMLButtonElement).disabled).toBe(false)
    expect(screen.getByRole('button', { name: '검색' }).getAttribute('aria-busy')).toBe('true')
    expect((await within(region).findByRole('status')).textContent).toBe('공고 결과를 갱신하는 중입니다')
    await act(async () => resolveSecond(Response.json({ ...catalog, programs: [{ ...program, id: 'seoul-1', title: '서울 창업 지원' }] })))
    expect(within(region).getByRole('link', { name: '서울 창업 지원' })).toBeTruthy()
    expect(within(region).queryByRole('link', { name: program.title })).toBeNull()
    expect(region.querySelector('[aria-busy="true"]')).toBeNull()
    await waitFor(() => expect(within(region).queryByRole('status')).toBeNull())
  })

  it('갱신에 실패하면 이전 목록을 유지하고 인라인 배너에서 다시 불러온다', async () => {
    const fetchMock = vi.fn().mockResolvedValueOnce(Response.json(catalog)).mockResolvedValueOnce(new Response('', { status: 503 }))
      .mockResolvedValueOnce(Response.json({ ...catalog, programs: [{ ...program, id: 'retry-1', title: '재시도 공고' }] }))
    vi.stubGlobal('fetch', fetchMock)
    start()
    await screen.findByRole('link', { name: program.title })
    fireEvent.click(screen.getByRole('radio', { name: '수출' }))
    ...
    expect(alert.textContent).toContain('이전 결과를 보여 드리고 있어요')
    expect(within(region).getByRole('link', { name: program.title })).toBeTruthy()
    fireEvent.click(within(alert).getByRole('button', { name: '다시 불러오기' }))
    await screen.findByRole('link', { name: '재시도 공고' })
    expect(fetchMock).toHaveBeenCalledTimes(3)
    expect(fetchMock.mock.calls[2][0]).toBe(fetchMock.mock.calls[1][0])
  })

  it('상세를 보고 돌아오면 캐시된 목록을 즉시 보여 주고 뒤에서 다시 조회한다', async () => {
    const fetchMock = vi.fn().mockImplementation(async (url: string) => Response.json(url.includes('/catalog?') ? catalog : program))
    vi.stubGlobal('fetch', fetchMock)
    start()
    await screen.findByRole('link', { name: program.title })
    fireEvent.click(screen.getByRole('link', { name: program.title }))
    await screen.findByRole('link', { name: '← 검색 결과로 돌아가기' })
    const catalogCallsBefore = fetchMock.mock.calls.filter(([url]) => String(url).includes('/catalog?')).length
    fireEvent.click(screen.getByRole('link', { name: '← 검색 결과로 돌아가기' }))
    // 응답을 기다리지 않고 캐시된 행이 바로 있고 스켈레톤은 없다.
    const region = screen.getByRole('region', { name: '필터 검색 결과' })
    expect(within(region).getByRole('link', { name: program.title })).toBeTruthy()
    expect(region.querySelector('[aria-hidden="true"] > div > div')).toBeNull()
    await waitFor(() => expect(fetchMock.mock.calls.filter(([url]) => String(url).includes('/catalog?'))).toHaveLength(catalogCallsBefore + 1))
    expect(within(region).getByRole('link', { name: program.title })).toBeTruthy()
  })

  it('적용된 필터 칩을 해제하면 그 조건만 빼고 첫 페이지부터 다시 검색한다', async () => {
    const fetchMock = vi.fn().mockImplementation(async (url: string) => {
      const page = Number(new URL(url).searchParams.get('page') ?? '1')
      return Response.json({ ...catalog, page, total: 13, totalPages: 2, programs: page === 2 ? [program] : Array.from({ length: 12 }, (_, index) => ({ ...program, id: `p1-${index}` })) })
    })
    vi.stubGlobal('fetch', fetchMock)
    start('/?mode=filter&' + new URLSearchParams({ region: '서울', category: '수출', status: 'UNKNOWN', page: '2' }))
    await screen.findByRole('link', { name: program.title })
    const chips = screen.getByRole('group', { name: '적용된 필터' })
    expect(within(chips).getAllByRole('button').map((button) => button.getAttribute('aria-label') ?? button.textContent)).toEqual([
      '지역: 서울 해제', '분야: 수출 해제', '접수 상태: 상태 미확인 해제', '모두 해제',
    ])
    fireEvent.click(within(chips).getByRole('button', { name: '지역: 서울 해제' }))
    await waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(2))
    const params = new URLSearchParams(screen.getByTestId('location').textContent!.split('?')[1])
    expect(params.get('region')).toBeNull()
    expect(params.get('category')).toBe('수출')
    expect(params.get('status')).toBe('UNKNOWN')
    expect(params.get('page')).toBeNull()
    fireEvent.click(screen.getByRole('button', { name: '모두 해제' }))
    await waitFor(() => expect(screen.getByTestId('location').textContent).toBe('/?mode=filter'))
    expect(screen.queryByRole('group', { name: '적용된 필터' })).toBeNull()
  })
