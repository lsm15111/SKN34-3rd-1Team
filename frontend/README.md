...
| `/pricing` | 헤더 | 무료·프로·팀 요금제 소개, 베타 기간 안내와 정식 전환 예정 가격, FAQ, 무료 검색 진입 |
| `/faq` | 헤더 | 도움말 항목 10건을 검색·분류·아코디언으로 보는 자주 묻는 질문. FAQ 구조화 데이터 포함 |
...
| `/app/faq` | 사이드바 | 같은 자주 묻는 질문을 사이드바 안에서 |
...
| `/app/profile` | 사이드바 계정 카드 메뉴 | 사업자등록번호 조회로 기업 등록(기업명·소재지·업종·설립연도·홈페이지)·기본정보 수정. 완성도와 체크리스트는 맨 위 요약 카드, 이 정보가 쓰이는 곳·공개 범위는 카드 제목 옆 `?` 도움말(한 칸 배치) |
...
- 머리에 무엇을 답하는 곳인지 한 줄로 붙여 대화가 길어져도 범위가 사라지지 않습니다.
- 추천 질문 3개(좁은 화면 2개)는 입력창 바로 위에 있고 지금 경로에 맞춰 화면이 가진 목록에서 고릅니다. 네트워크를 쓰지 않습니다.
...
- AI가 만든 답변은 본문 아래 `◈ AI 생성 · 도움말 N항목 근거` 한 줄과 번호 인용으로 출처를 밝히고, 인용을 누르면 요약과 갱신일을 펼칩니다.
...
공고당 모집글은 하나이며 마감일은 접수 마감 전날까지입니다. 모집글 저장·숨김·신고처럼 아직 만들지 않은 동작은
비활성 버튼으로 자리만 두지 않고 화면에서 뺐습니다. 프로필의 선택은 화면 안에서만 유지되고 추천에 전달되지 않습니다.
...
선택 메뉴는 연한 초록색 배경·초록색 글자로 구분하며, 모든 메뉴가 실제 화면을 가집니다.
...
지금은 세 요금제의 기능을 모두 결제 없이 쓸 수 있어 카드마다 검색으로 가는 버튼만 두고, 프로·팀에는 정식
전환 시 **예정 가격**을 함께 적습니다. 예정 가격은 아직 확정 정책이 아니며 근거와 대안은
[요금제 검토](../docs/pricing-proposal-20260912.md)에 있습니다. 결제·구독·카드 등록·사용량 제한을
구현한 화면이 아니고, 페이지 조회와 FAQ 열기에는 API 요청이 발생하지 않습니다.

자주 묻는 질문(`/faq`, `/app/faq`)은 도움말 항목 한 벌을 그대로 읽어 분류·검색·아코디언으로 보여 줍니다.
챗봇이 답하는 내용과 같은 원본을 쓰므로 두 화면의 안내가 어긋나지 않으며, 검색은 화면 안에서만 걸러
네트워크를 쓰지 않습니다. 검색 엔진과 AI 도우미가 인용할 수 있도록 FAQ 구조화 데이터를 함께 싣습니다.
