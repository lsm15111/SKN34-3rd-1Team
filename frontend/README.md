...
데이터가 아직 하나도 없는 첫 진입은 `presentation/shared/loading`의 두 가지로만 표시합니다. 회색 한 줄 텍스트나 `<h1>`로 "불러오는 중"을
쓰지 않습니다.

| 상황 | 컴포넌트 | 쓰는 곳 |
|---|---|---|
| 목록·상세처럼 **들어올 내용의 구조를 아는** 첫 진입 | `LoadingRegion` + `Skeleton*` (`SkeletonDetail`·`SkeletonRows`·`SkeletonCards`·`SkeletonText`) | 공고 상세, 파트너 모집 상세·수정·공개 상세, 제안함, 관심 공고함(2), 관리자 계정 상세, 기업 프로필, 중복 검토 목록·상세·실행 결과, 신청 준비 목록·양식 |
| 구조를 그릴 자리가 없는 **작은 영역·대화 상자·폼 안** | `LoadingState` (회전 아이콘 + 한 줄) | 리포트 설정, 검토의 공고 검색, 관심 공고 선택 대화 상자, 이미 목록이 있는 상태의 검토 목록 갱신 |
| 조건 변경으로 **이전 목록을 유지**하며 다시 읽는 중 | `shared/data/ResultsRegion` | 필터 검색·파트너 목록·관리자 목록 |

- 스켈레톤 조각은 `aria-hidden`, 안내 문장은 `role="status"`(sr-only), 영역은 `aria-busy="true"`. 문장은 기존 문구를 그대로 둡니다.
- 움직임은 `motion-safe:animate-pulse`뿐이라 감속 설정 사용자는 정지된 회색 자리만 봅니다.
- 스켈레톤은 실제 행·카드와 같은 격자 클래스(`gridClassName`)와 비슷한 높이를 써서 내용이 오면 자리만 채워지게 합니다.
- 버튼·입력·모달, 10초 넘게 걸리는 AI 작업(채팅 검색·원문 질문·문서 분석)에는 스켈레톤을 쓰지 않습니다. 그쪽은 기존 진행 표시를 유지합니다.
