...
잘못된 비밀번호·정지 계정·시도 제한(429)을 구분해 안내합니다. 개발 빌드의 헤더에는 `개발 로그인 · 관리자`·`회원`·`기업`
버튼이 있어 회원가입 없이 단계별 시드 계정으로 들어갈 수 있습니다(Core의 `ACCOUNT_DEV_LOGIN_ENABLED`). `기업`은 예시
기업이 등록된 회원이라 모집글 작성·제안·제안함을 바로 볼 수 있고, `infrastructure/dev-seed.sql`을 넣어 두면 이 계정이
쓴 모집글과 받은·보낸 제안 목데이터가 함께 보입니다.
