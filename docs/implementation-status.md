...
| 어드민 모집글 제어 | 구현됨 | `/admin/recruitment-posts` 모든 상태 목록(상태 필터), 사유 필수 숨김·강제 마감, 숨김 해제. 숨긴 글은 작성 기업 외 404, 제안 불가 |
| 어드민 대시보드·수집 실행 화면 | 미구현 | 범위 밖 |
| 파트너 모집글 API | 구현됨 | 공고에 묶인 모집글 등록·수정·조기 마감·목록(모집 중, 마감 임박순)·상세·내 글. 상태는 조회 시 계산, 연락처 문구 거부. 상세에 받은 제안 수·내 제안 상태 포함 |
| 파트너 모집글 화면 | 구현됨 | `/partners` 목록(공고 필터·페이지), 상세(연결 공고·작성 기업·조기 마감), 작성·수정(공고 검색 선택, 서버와 같은 폼 규칙), 내 모집글. 공고 상세·채팅 사이드바에서 진입 |
| 참여 제안 API | 구현됨 | 다른 기업의 모집 중 글에 기업당 한 번 제안, 받은/보낸 제안 목록, 수락·거절·철회(대기 중만), 7일 만료·글 종료 상태 계산, 수락 시에만 상대 담당자 이메일 공개 |
| 참여 제안 화면 | 구현됨 | 모집글 상세의 제안 폼(500자·연락처 거부·기업당 한 번), 내 제안 상태·철회, 작성 기업의 받은 제안 수락·거절, 내 활동의 보낸 제안 탭, 수락 시에만 상대 담당자 이메일 표시 |
...
| 스키마 관리 | 구현됨 | Flyway V1 공고, V2 동기화 세대, V3 공고별 공식 원문, V4 공개 스냅샷 준비 상태, V5 계정·기업·로그인 세션 테이블, V6 계정 역할, V7 파트너 모집글, V8 참여 제안 |
