...
| 회원가입·로그인 API | 구현됨 | 이메일·비밀번호·사업자등록번호로 가입(`POST /auth/signup`, Bizno 재확인), 로그인(`/login`), 내 계정(`/me`), 로그아웃(`/logout`). BCrypt 해시, DB 저장 세션 토큰(기본 30일), 실패는 409·422·401 |
| 회원가입·로그인 화면 | 구현됨 | `/signup`은 사업자등록번호 확인 → 이메일·비밀번호·약관 동의, `/login`은 이메일·비밀번호. 채팅 헤더에 회사명·로그아웃 표시 |
| 로그인 상태 유지 | 구현됨 | 세션 토큰을 `localStorage`에 저장하고 앱 진입 시 `GET /auth/me`로 복원. XSS 대비 CSP는 후속 과제 |
| 비로그인 검색 제한 | 구현됨 | 브라우저 저장 기준 3회 뒤 가입·로그인 안내 모달. 로그인하면 해제. 서버 측 제한은 없음 |
| 관리자 역할·어드민 회원 목록 | 구현됨 | V6 `account.role`(SQL로 지정), `GET /admin/accounts` 이메일 검색·페이지, 세션 강제 종료. `/admin/accounts` 화면은 관리자만 진입 |
| 어드민 모집글 제어·대시보드 | 미구현 | 모집글 제어는 [파트너 모집 계획](partner-recruitment-plan.md) P6, 대시보드·수집 실행 화면은 범위 밖 |
| 파트너 모집글 API | 구현됨 | 공고에 묶인 모집글 등록·수정·조기 마감·목록(모집 중, 마감 임박순)·상세·내 글. 상태는 조회 시 계산, 연락처 문구 거부. 화면·제안은 미구현 |
| 파트너 모집글 화면 | 구현됨 | `/partners` 목록(공고 필터·페이지), 상세(연결 공고·작성 기업·조기 마감), 작성·수정(공고 검색 선택, 서버와 같은 폼 규칙), 내 모집글. 공고 상세·채팅 사이드바에서 진입 |
| 참여 제안 | 미구현 | [파트너 모집 계획](partner-recruitment-plan.md) P4~P5 |
| 이메일 인증·비밀번호 재설정·로그인 시도 제한 | 미구현 | 메일 발송·복구 흐름·서버 측 제한 없음 |
...
| 스키마 관리 | 구현됨 | Flyway V1 공고, V2 동기화 세대, V3 공고별 공식 원문, V4 공개 스냅샷 준비 상태, V5 계정·기업·로그인 세션 테이블, V6 계정 역할, V7 파트너 모집글 |
