...
| Google·카카오 소셜 로그인 | 구현됨 | `GET /api/v1/auth/oauth/providers`, `GET .../{google\|kakao}/start`, `GET .../callback`. 서버 리다이렉트형 Authorization Code(Google은 PKCE)로 Core가 토큰을 교환하고 사용자 정보로 회원번호·이메일을 확인. 제공처 회원번호로 계정을 찾고, 없으면 제공처가 인증한 이메일에 한해 기존 계정에 연결(미인증 이메일은 연결 거부), 그도 없으면 비밀번호 없는 계정 생성. 세션은 이메일 로그인과 같은 쿠키(30일). 클라이언트 ID가 설정된 제공처만 로그인·회원가입 화면에 버튼으로 표시되고 결과는 `/oauth/callback` 화면이 받음. 계약은 [계정·인증 계약](account-auth-contract.md#소셜-로그인-google카카오) |
