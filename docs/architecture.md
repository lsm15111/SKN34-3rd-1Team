...
- MSIT 신청 기간(`MSIT_PERIOD_EXTRACTION_ENABLED`, 기본 비활성): `MsitApplicationPeriodExtractionWorker`가 공개 공고를
  ...
  결과는 `support_program_period_extraction`(V40)에 저장하고 공개 행의 기간·안내 문구를 즉시 갱신하며, MSIT 동기화는 저장된 기간을 다시 병합해 null로 덮어쓰지 않습니다.
  ...
  규칙 버전 2(V41)는 같은 첨부에서 `사업 목적·개요`와 `신청 자격·지원 대상` 표제 뒤 원문을 다음 표제 전까지 발췌합니다
  (`SupportProgramNoticeSectionExtractorHelper`, 최대 500/400자, 목차·메타 줄·연락처·대상 명사 없는 구간 제외, 재작성·대상 분류 없음).
  발췌는 MSIT 검색 색인 텍스트(`지원대상`·`내용`)에 들어가므로 워커는 저장만 하고 색인을 먼저 준비하는 MSIT 동기화가 병합합니다.
  워커가 즉시 바꾸는 기간·기간 안내 문구는 색인 텍스트에 없어 AI 검색의 색인 해시 검증과 충돌하지 않습니다(`MsitNoticeContentHelperTest`).
  규칙 버전이 바뀌어도 새 결과로 바뀌기 전까지 기존 결과를 계속 병합합니다. 대상 분류(기업/대학·연구기관)는 R&D 공고의 법령 목록이
  잘려 기업 참여 가능 공고를 좁힐 위험이 있어 넣지 않았습니다.
