...
AssistantMessageController (공유 요청 한도 + 로그인 회원은 주소당 추가 한도)
  ...
  → AiAssistantClient → AI Service POST /internal/v1/assistant/answers
      ...
      → AssistantService: 인용은 요청 카탈로그 안, 카드와 실행 제안은 이번 실행의 도구 결과 안(제목·경로는 도구 결과로 다시 만듦),
        자료를 읽지 않은 회원 자료 답은 버림
  → Core 재검증(의도별 필드·카탈로그 인용·카드 경로·이동 허용 목록) + AssistantActionService(대상 소유·현재 상태 확인,
    버튼 문구 생성) → 템플릿 답·로그인 안내
...
실행은 가이드가 하지 않습니다. 모델은 종류와 대상만 담은 제안(`actions`, 최대 2개)을 내고, Core가 그 대상이 지금도
이 계정의 것인지·상태가 맞는지(담기는 담기지 않은 공고만, 단계 변경은 지금과 다른 단계만) 확인한 뒤 버튼 문구를 직접
만들어 내려보냅니다. 화면은 사용자가 그 버튼을 눌렀을 때만 기존 기능 API를 부릅니다(담기·빼기 `POST/DELETE
/api/v1/me/saved-programs`, 진행 단계 `PUT /api/v1/application-preparations/{id}/progress-stage`, 중복 검토 실행
`POST /api/v1/combination-reviews/{id}/runs`). 낙관적 잠금이 필요한 변경은 누른 직후 현재 개정 번호를 다시 읽어 보냅니다.
신청 문서 준비 시작은 양식을 고르는 단계가 있어 실행하지 않고 그 공고가 선택된 작성 화면을 여는 이동 버튼입니다.
제안 수락·거절, 모집글 발행, 리포트 메일 동의, 계정 변경은 가이드가 대신 누르지 않고 화면으로 안내만 합니다.

답은 만들어지는 동안 화면에 흘러갑니다. 브라우저가 `Accept: text/event-stream`으로 물으면 같은 경로가 SSE로 답하고,
Core가 AI Service의 `/internal/v1/assistant/answers/stream`을 중계합니다. 이벤트는 `status`(무엇을 하는 중인지),
`text`(답변 문장 조각), `final`(검증을 마친 답) 세 가지입니다. 모델 호출 수와 토큰은 한 번에 받을 때와 같습니다.
조각은 구조화 출력 JSON에서 `answer` 값만 뽑아낸 것이라 카드·인용·실행 제안은 섞이지 않고, 검증 전이므로 화면은
"만드는 중"으로만 보여 주다가 `final`이 오면 그 답으로 바꿉니다(Core가 템플릿 답으로 대신하는 경우 문장이 달라집니다).
첫 이벤트를 보내기 전의 실패는 평소처럼 상태 코드로 나가고, 보낸 뒤의 실패만 `error` 이벤트가 되어 실패 말풍선이 됩니다. 관심 공고 묶음 질문은 목록(제목·기관·마감·상태)만 읽고, 원문이 필요한 내용은
