...
| 3. 스트리밍 | 완료 | `e90dfd8` | AI Service 1,326(가이드 138), Core 어시스턴트 105, 프런트 1,298, 실제 스택 SSE 5문항 |
...
3단계에서 설계와 달라진 점:
- 스트리밍은 새 경로가 아니라 같은 `POST /api/v1/assistant/messages`의 `Accept: text/event-stream` 응답입니다.
  중간에 Accept가 바뀌어 JSON이 와도 화면이 그대로 읽습니다.
- 답변 문장은 구조화 출력 JSON에서 `answer` 값만 뽑아 조각으로 보냅니다(`app/assistant/streaming.py`).
  Core가 템플릿 답으로 바꾸는 경우(비로그인 회원 자료 질문 등) 조각과 확정 문장이 다를 수 있어, 화면은 조각을
  대화에 남기지 않고 `final`이 오면 그 문장으로 바꿉니다.
- 진행 표시는 `status` 이벤트(생각하는 중·자료 확인 중) 한 줄입니다. 도구 이름 같은 내부 값은 내보내지 않습니다.

3단계 실측(2026-09-19, 로컬 Docker, 실제 OpenAI `gpt-5.6-luna`/low):
- 첫 이벤트(진행 표시) 0.01~0.44초. 첫 글자는 도구를 쓰지 않는 사용법 질문 0.8~2.3초, 회원 자료를 읽는 질문 2.3~3.7초,
  전체 1.3~4.9초입니다. 설계의 "첫 글자 2초 이내"는 도구를 쓰지 않는 질문에서만 지켜집니다(남은 시간은 모델이 생각하는 시간).
- 조각으로 나간 문장은 5문항 중 4문항에서 확정 답과 같았고, 비로그인 회원 자료 질문은 조각이 아예 없이 Core 안내 문구만 나갔습니다.
- 브라우저에서도 글자가 늘어나는 것을 확인했습니다(1.2초에 "검색 결과의 점수", 1.7초에 전체 문장). 모델 호출 수·토큰은 이전과 같습니다.
