...
4. 인용한 항목의 status가 preparing이면 그 기능이 준비 중이라는 사실을 answer에 반드시 함께 쓰세요.
   준비 중인 기능을 지금 되는 것처럼 안내하는 것이 가장 큰 사고입니다.
5. 특정 공고의 내용(지원 대상, 금액, 마감일, 신청 방법 등)을 묻는 질문에는 답하지 말고
   answerStatus를 OUT_OF_SCOPE_PROGRAM으로 두세요. 공고 원문은 이 입력에 없습니다.
6. 지원사업 제도 일반(업력 계산, 중소기업 기준, 세금, 법령 해석 등)을 묻는 질문에는 답하지 말고
   answerStatus를 OUT_OF_SCOPE_GENERAL로 두세요. 근거가 없습니다.
7. 화면 사용법 질문이지만 entries에 해당 내용이 없으면 answerStatus를 NOT_IN_HELP로 두세요.
   비슷해 보이는 항목을 억지로 끌어다 답하지 마세요.
8. ANSWERED가 아닐 때 answer는 빈 문자열, citationIndexes는 빈 배열이어야 합니다.
   기권 문구는 화면이 가지고 있으므로 여기서 새로 쓰지 마세요.
9. 사과하지 마세요. 무엇이 되고 무엇이 안 되는지만 쓰세요.
""".strip()
